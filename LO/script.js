...
// ==========================================================================
// 4. SISTEM UPLOAD GAMBAR SUPER PRO (100% BEBAS BUG & HAPUS FILE EXPLORER NGASAL)
// ==========================================================================
document.querySelectorAll('.img-upload-wrapper').forEach(wrapper => { 
    ...
    // PERBAIKAN FATAL: Memutus jebakan label HTML bawaan
    if (labelElement) {
        labelElement.removeAttribute('for');
        labelElement.addEventListener('click', function(e) {
            e.preventDefault(); // Mencegah 1x klik membuka dialog otomatis
        });
    }
    ...
    if (input) { 
        ...
        // Mencegah klik dari input merambat memicu error
    ...
    // LOGIKA 1: KLIK 1X (Murni hanya mengaktifkan kotak hijau, dialog Explorer DILARANG kebuka)
    ...
    // LOGIKA 2: KLIK 2X / DOUBLE CLICK (Ini yang resmi bertugas membuka Explorer)
    wrapper.addEventListener('dblclick', function(e) { 
        if (input) input.click(); 
    }); 

    // LOGIKA 3: HAPUS GAMBAR VIA DELETE / BACKSPACE
...
// HILANGKAN FOKUS SAAT KLIK DI LUAR
...
document.getElementById('btnExport').addEventListener('click', function() { 
    ...
    const wadahSizes = originalImgs.map(img => { 
        ...
        return { w: rect.width || 100, h: rect.height || 135 }; 
    }); 
    ...
    Promise.all(imagePromises).then(() => { 
        ...
        html2canvas(target, { 
            ...
            onclone: (cloned) => { 
                ...
                cloned.querySelectorAll('.id-row input, .size-table input, .color-table input, .notes-box textarea, .detail-text-box textarea, .detail-box-input-row input').forEach((i, index) => { 
                    ...
                    if (i.tagName === 'TEXTAREA') { 
                        if(i.closest('.detail-text-box')) { 
                            v.style.cssText = "padding:4px; font-weight:bold; font-size:16px; font-family:sans-serif; text-align:center; width:100%; height:100%; display:flex; align-items:center; justify-content:center;"; 
                        } else { 
                            v.style.cssText = "padding:30px; font-weight:bold; font-size:24px; white-space:pre-wrap; text-align:left; font-family:sans-serif; width:100%; box-sizing:border-box;"; 
                        } 
                        ...
                        v.style.cssText = `text-align:center; font-weight:bold; font-size:12px; display:flex; align-items:center; justify-content:center; width:100%; height:100%; color:#000; background:${bgComputed};`; 
                    } else if (i.closest('.size-table') && i.style.fontWeight === 'bold') { 
                        v.style.cssText = "text-align:center; font-weight:900; font-size:14px; display:flex; align-items:center; justify-content:center; width:100%; height:100%; color:#000;"; 
                    } else if (i.closest('.detail-box-input-row')) { 
                        v.style.cssText = "text-align:center; font-weight:900; font-size:15px; display:flex; align-items:center; justify-content:center; width:100%; height:100%; color:#000;"; 
                    } else { 
                        v.style.cssText = "text-align:center; font-weight:bold; font-size:14px; display:flex; align-items:center; justify-content:center; width:100%; height:100%; color:#000;"; 
                    } 
            ...
            f4Canvas.height = Math.round(canvas.width * (33 / 21)); 
            
            const ctx = f4Canvas.getContext('2d'); 
            ctx.fillStyle = "#ffffff"; 
            ctx.fillRect(0, 0, f4Canvas.width, f4Canvas.height); 
            ctx.drawImage(canvas, 0, 0, f4Canvas.width, f4Canvas.height); 
            
            f4Canvas.toBlob(blob => { 
